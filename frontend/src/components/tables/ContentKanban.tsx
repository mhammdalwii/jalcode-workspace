              ...
              <div className="p-3 flex flex-col gap-3 min-h-37.5">
                {columnContents.length === 0 ? (
                  ...
                  columnContents.map((content) => {
                    ...
                    // 🚀 PERBAIKAN: Pastikan platform adalah Array
                    ...
                    return (
                      <div key={content.id} className={`bg-white p-4 rounded-xl shadow-sm border hover:shadow-md transition group ${isRejected ? "border-red-200" : "border-slate-200"}`}>
                        <div className="flex justify-between items-start mb-2">
                          {/* 🚀 PERBAIKAN: Mapping Platform Array */}
                          <div className="flex flex-wrap gap-1">
                            {platforms.map((plat, idx) => {
                              const platformStyle = getPlatformStyle(plat);
                              return (
                                <span key={idx} className={`flex items-center gap-1.5 px-2 py-1 text-[10px] font-bold uppercase tracking-wider rounded-md ${platformStyle.bg} ${platformStyle.text}`}>
                                  {platformStyle.icon} {plat}
                                </span>
                              );
                            })}
                          </div>
                        ...
                        <div className="flex justify-between items-end border-t border-slate-100 pt-3">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs font-medium text-slate-700">{content.pics && content.pics.length > 0 ? content.pics.map((p) => p.name.split(" ")[0]).join(", ") : "Belum ada"}</span>
                          </div>
                        ...
                        <div className="mt-3 pt-3 border-t border-slate-100">
                          {isWaitingApproval ? (
                            <div className="w-full text-[11px] font-bold text-amber-600 bg-amber-50 border border-amber-200 rounded py-1.5 px-2 text-center flex items-center justify-center gap-1">
                              <Clock size={12} /> Menunggu Founder
                            </div>
                          ) : (
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-5">
              ...
              <textarea
                ...
                rows={4}
                placeholder="Contoh: Kurang relevan..."
                value={rejectReason}
